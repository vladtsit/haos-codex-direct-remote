...
The mount point is `/homeassistant` inside the container (some HA/Supervisor versions expose it as `/homeassistant_config` instead — check with `ls /homeassistant /homeassistant_config` if one of them doesn't exist). Home Assistant mounts it `0700 root:root`, unreadable by the unprivileged `codex` user the add-on otherwise runs as. Since it's a read-only bind mount, its permissions can't be relaxed with `chmod`/`setfacl` from inside the container, so the image grants `codex` passwordless, unrestricted `sudo` instead (`/etc/sudoers.d/codex`) — read it with `sudo cat /homeassistant/configuration.yaml`, `sudo ls /homeassistant`, etc. Be aware this `sudo` grant is not scoped to just that directory: it gives `codex` (and therefore anything Codex executes) full root inside the container. That's consistent with this add-on's existing threat model (single-tenant, your own container, no host privileges or `full_access`), but it's broader than "read `/config`" — remove the sudoers rule in your own fork if you want Codex confined to the `codex` user's own permissions and are fine losing `/config` readability as a result.
