...
The add-on works around this by mirroring it: at startup (and every 60s afterwards) a root-owned step copies the mount to `/data/home/ha_config_ro` (i.e. `~/ha_config_ro` for the `codex` user), owned by and readable by `codex` with no elevation needed. Read it directly, e.g. `cat ~/ha_config_ro/configuration.yaml`. This mirror can lag up to ~60s behind the live HA config, and it's a snapshot, not a live view — for anything security-sensitive re-read it after a `sync_ha_config_mirror` cycle rather than trusting a cached copy blindly. The sync uses `rsync -a --delete` (only changed files touched, not a full rewrite each cycle) and excludes `.storage/` (HA's own internal auth tokens/state), the recorder database, `www/`, `tts/`, `media/`, and `backups/` — none of that is needed to read `configuration.yaml`/automations/scripts, and `.storage/` in particular is more sensitive than typical YAML config, so it's deliberately left out of what's exposed to the mirror. It's also excluded from Supervisor backups (it's a disposable cache, regenerated every ~60s, not source data worth persisting).

A passwordless, unrestricted `sudo` grant for `codex` (`/etc/sudoers.d/codex`) is also present and works from an interactive SSH shell (e.g. `sudo cat /homeassistant/configuration.yaml`), but **not** from inside a Codex session's own tool calls — Codex's exec sandbox sets `no_new_privileges`, which silently blocks the setuid `sudo` binary from escalating there. Use the `~/ha_config_ro` mirror from Codex sessions instead. Be aware the `sudo` grant itself is not scoped to just `/config`: it gives `codex` (and therefore anything running under it outside the sandbox, e.g. SSH) full root inside the container. That's consistent with this add-on's existing threat model (single-tenant, your own container, no host privileges or `full_access`), but remove the sudoers rule in your own fork if you want `codex` confined to its own permissions everywhere.
